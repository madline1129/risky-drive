...
- Preserve the L0 scene identity where possible: use the L0 map, weather, actor types, ego-relative distances, relative lateral/longitudinal offsets, and lane relationships.
- Treat L0 absolute coordinates as best-effort spawn hints, not hard requirements. If a L0 transform is not spawnable, preserve the relative geometry around the actual ego pose instead of failing solely because the absolute `(x, y)` differs.
- If a L0 transform is occupied, move minimally along the lane or upward in z; if that still fails, relocate the scene while keeping the same ego-to-primary relative geometry. Do not switch to an unrelated map region or unrelated risk template.
...
- Follow `scenario_config.spawn_policy`. If exact ego or primary L0 spawning fails after waypoint/navmesh and z-offset retries, relocate the entire scene to a valid ego spawn/waypoint, recompute primary/background actor poses relative to the actual ego pose, save images, and record relocation metadata plus relative-position error in `event_trace.json`. Do not fail before the frame loop solely because an exact L0 absolute pose cannot spawn.
- CARLA waypoint traversal distances must be non-negative. Never call `waypoint.next(-distance)`; use `waypoint.previous(abs(distance))` when available or compute a backward offset with the actor/waypoint forward vector.
