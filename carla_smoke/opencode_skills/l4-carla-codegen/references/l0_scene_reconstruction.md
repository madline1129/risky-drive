...
3. Spawn the ego vehicle near `ego.location` and `ego.rotation` when possible. Match `ego.type_id` if possible, otherwise use a similar vehicle blueprint.
4. For front-vehicle events, spawn `nearest_front_actor` at its L0 relative position to ego first; use absolute `location`/`rotation` only when it is directly spawnable. Match `type_id` if possible.
...
- If exact ego or primary actor coordinates are not spawnable, relocate the scene and preserve ego-relative longitudinal/lateral offsets. Absolute coordinate mismatch is acceptable when relative geometry and risk visibility are preserved.
...
Do not choose a random or unrelated spawn point when L0 pose or relative-offset data is present.
