...
硬性原则：
- 不要改变 L3 的核心事故链。
- 如果输入里已有 actor_list / selected_actor / primary_perturbation_object / perturbation_target / object_registry，必须优先继承。
- source="l0_actor" 的 primary_object 必须保留原始 actor_id/type_id/location/rotation/relative_* 字段；禁止改写成 generated_actor。只有上游明确没有原始对象时，才允许生成新 actor。
...
- vulnerable_actor_intrusion 的 primary_object 必须是 pedestrian/walker/cyclist 或 generated vulnerable actor；如果上游 selected_actor 是 L0 原始行人/骑行者，必须使用该 L0 actor，不能新生成替代行人。
...
def chain_object_registry(chain):
    for key in ("object_registry", "chain_participants"):
        ...
        if isinstance(value, (dict, list)):
            return value
    collected = {}
    for key in ("primary_perturbation_object", "selected_actor", "perturbation_target", "risk_object", "primary_object"):
        value = chain.get(key) if isinstance(chain, dict) else None
        if isinstance(value, dict):
            collected[key] = value
...
def chain_actor_list(chain):
    if not isinstance(chain, dict):
        return None
    value = chain.get("actor_list")
...
def merge_plan_with_agent_output(base_plan, agent_output):
        ...
        if isinstance(primary, dict):
            if primary.get("source") == "l0_actor":
                if primary.get("actor_id") is not None:
                    merged["primary_actor_id"] = primary.get("actor_id")
                merged["primary_actor_source"] = "l0_actor"
                if primary.get("kind") is not None:
                    merged["primary_actor_kind"] = primary.get("kind")
                if primary.get("type_id") is not None:
                    merged["primary_actor_type_id"] = primary.get("type_id")
                if primary.get("role") is not None:
                    merged["primary_actor_role"] = primary.get("role")
            else:
                if primary.get("actor_id") is not None:
    ...
    base_registry = (base_plan or {}).get("object_registry")
    if isinstance(base_registry, dict):
        base_primary = base_registry.get("primary_object")
        if isinstance(base_primary, dict) and base_primary.get("source") == "l0_actor":
            merged["primary_actor_id"] = base_primary.get("actor_id", merged.get("primary_actor_id"))
            merged["primary_actor_source"] = "l0_actor"
            merged["primary_actor_kind"] = base_primary.get("kind", merged.get("primary_actor_kind"))
            merged["primary_actor_type_id"] = base_primary.get("type_id", merged.get("primary_actor_type_id"))
    return merged
...
def build_risk_object_spec(plan, scene_reconstruction):
    ...
    if scenario_type == "vulnerable_actor_intrusion":
        ...
        actor = l0_actor if is_vulnerable_actor(l0_actor) else None
        source_declares_l0 = plan.get("primary_actor_source") == "l0_actor"
        use_l0_actor = actor is not None or source_declares_l0
        primary_actor_id = (actor or {}).get("id", actor_id) if use_l0_actor else "generated_vulnerable_actor"
        primary_kind = (actor or {}).get("kind") or plan.get("primary_actor_kind") or plan.get("actor_type", "walker")
        primary_type_id = (
            (actor or {}).get("type_id")
            or plan.get("primary_actor_type_id")
            or ("walker.*" if plan.get("actor_type", "walker") == "walker" else plan.get("actor_type", "walker"))
        )
        start_world = actor_location(actor) or path.get("start_world")
        ...
        base.update(
            {
                "primary_object": {
                    ...
                    "kind": primary_kind,
                    "source": "l0_actor" if use_l0_actor else "generated_actor",
                    "actor_id": primary_actor_id,
                    "type_id": primary_type_id,
                    "initial_location": start_world,
                    "initial_rotation": actor_rotation(actor) or ego_rotation,
                    "initial_relative_lateral_m": (actor or {}).get("relative_lateral_m"),
                    "initial_relative_longitudinal_m": (actor or {}).get("relative_longitudinal_m"),
                    "initial_distance_m": (actor or {}).get("distance_m"),
                    "l0_actor_snapshot": actor,
                },
                "geometry": {
                    ...
                    "end_world": end_world,
                    "path_world": [point for point in (start_world, end_world) if point],
                    "lane_crossing_required": True,
                    "world_origin_is_forbidden": True,
                    "start_is_original_l0_actor_pose": bool(use_l0_actor),
                },
                "action": {
                    "mode": (
                        "move_existing_vulnerable_actor_across_ego_lane_after_trigger"
                        if use_l0_actor
                        else "walk_or_cycle_across_ego_lane_after_trigger"
                    ),
                    "speed_mps": plan.get("speed_mps", 2.2),
                    "must_cross_ego_lane_centerline": True,
                    "must_approach_ego": True,
                    "do_not_spawn_replacement_primary_actor": bool(use_l0_actor),
                },
                "success_criteria": {
                    "primary_actor_id_must_match": primary_actor_id if use_l0_actor else None,
                ...
                "forbidden_substitutions": [
                    ...
                    "generated_replacement_for_l0_vulnerable_actor" if use_l0_actor else None,
                ],
            }
        )
        base["forbidden_substitutions"] = [item for item in base["forbidden_substitutions"] if item]
        return base
...
def build_physical_task(plan, scene_reconstruction, risk_object_spec=None):
    ...
    primary_source = risk_primary.get("source") or ("l0_actor" if primary_actor else "generated_actor")
    ...
    task = {
        ...
        "primary_actor": {
            ...
            "l0_actor_snapshot": primary_actor or risk_primary.get("l0_actor_snapshot"),
        },
        "ego_actor": {
            "source": "l0_ego",
    ...
    if scenario_type == "side_vehicle_intrusion":
        ...
        if primary_source == "l0_actor":
            task["action"]["do_not_spawn_replacement_primary_actor"] = True
            task["action"]["must_match_original_l0_actor"] = True
...
def build_config(
    ...
    return {
        ...
        "actor_list": chain_actor_list(chain),
        "risk_object_spec": risk_object_spec,
...
- For vulnerable_actor_intrusion, use the original L0 pedestrian/walker/cyclist when risk_object_spec.primary_object.source is "l0_actor"; only spawn a walker/cyclist when the primary object is explicitly generated.
