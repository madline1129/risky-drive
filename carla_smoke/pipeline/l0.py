...
L1_NODE_COUNT = 4
LIKELY_L1_NODE_COUNT = 2
FORCED_L1_FAMILY_IDS = {"rare_composite_risk", "weather_visibility_risk"}
KNOWN_L1_FAMILY_IDS = {
    "lead_vehicle_risk",
    "vru_risk",
    "side_vehicle_risk",
    ...
    "ego_action_risk",
    "rare_composite_risk",
    "weather_visibility_risk",
    "intersection_signal_risk",
    "oncoming_vehicle_risk",
    "merge_yield_risk",
    "wrong_way_u_turn_risk",
}
...
任务：
- 结合 L0 中的 ego、weather、actors、source，输出 4 个 L1 风险家族节点。
- rank 1-2：选择当前 L0 中最可能发生、最有状态证据支撑的两个风险薄弱环节。
- rank 3：由你基于 L0 想象一个低频但物理上合理、可由后续 CARLA 近似实现的不常见风险。它可以是复合遮挡、非典型车辆动作、异常物体/VRU暴露等；“前车突然左转后暴露行人”只是例子，不要固定复用。
- rank 4：固定输出一个天气/光照扰动风险，语义是“环境突然变成黑夜或可见度显著下降”。L1 只写风险家族和脆弱点，不在这里生成执行参数。
...
- 如果 L0/source 显示路口、junction、traffic_light_state、红灯/绿灯/信号灯，rank 1-2 必须优先考虑 intersection_signal_risk、oncoming_vehicle_risk、merge_yield_risk 中是否存在更强证据；典型薄弱环节包括横向来车闯红灯、支路/让行车辆冲出、对向左转抢行、右侧车辆进入自车路径。
...
硬性要求：
- l1_risk_predictions 必须正好 4 项，rank 从 1 到 4。
- rank 1-2 必须是当前 L0 中证据最强、可能性最大的两个风险。
- rank 3 必须是模型想象出的不常见风险，risk_family 必须为 rare_composite_risk；name、evidence、weakness_reason 不能固定套用“前车左转暴露行人”。
- rank 4 必须是天气/光照扰动风险，risk_family 必须为 weather_visibility_risk。
- rank 1-2 的 risk_family 必须优先来自 risk_family_candidates；rank 3-4 使用上面指定的固定 risk_family。
- 不要输出 risk_type_id、primary_action_primitive_id、risk_library_candidate、actor_list、selected_actor、primary_perturbation_object、legacy_scenario_type、matched_actor_id。
...
def fallback_risks_from_state(l0_state):
    ...
    for name, family, evidence in fallback:
        if len(risks) >= LIKELY_L1_NODE_COUNT:
            break
        risks.append(
            {
                "level": "L1",
    ...
    risks = risks[:LIKELY_L1_NODE_COUNT]
    risks.extend(forced_l1_tail_risks(l0_state))
    for idx, risk in enumerate(risks[:L1_NODE_COUNT], start=1):
        risk["rank"] = idx
    return risks[:L1_NODE_COUNT]


def forced_l1_tail_risks(l0_state):
    weather = l0_state.get("weather", {}) if isinstance(l0_state, dict) else {}
    ...
    return [
        {
            ...
            "name": "低频复合交通扰动脆弱点",
            "risk_type": "物理风险推测",
            "risk_family": "rare_composite_risk",
            "visibility": "纯推测",
            "risk_level": "低",
            "evidence": "模型不可用时的低频风险兜底槽位；L0单帧不要求直接观测完整链条",
            "weakness_reason": "该槽位用于覆盖低频但高危的非典型组合风险，例如遮挡关系突变、异常物体暴露、非典型车辆/VRU运动等；具体触发事件由 L2 继续细化。",
            "boundary": "L1只识别风险家族和薄弱环节，不生成触发事件、risk_type、动作原语或具体扰动物体",
        },
        {
            "level": "L1",
            ...
            "name": "光照突变导致可见度下降脆弱点",
            "risk_type": "物理风险推测",
            "risk_family": "weather_visibility_risk",
            "visibility": "纯推测",
            "risk_level": "低",
            "evidence": f"专项保留的天气扰动槽位；current_weather={weather}; source.frame={source.get('frame')}",
            "weakness_reason": "环境突然变为黑夜或可见度显著下降时，自车对前方车辆、行人和障碍物的观察余量会下降。",
            "boundary": "L1只识别风险家族和薄弱环节，不生成触发事件、risk_type、动作原语或具体扰动物体",
        },
    ]
...
def ensure_required_family_candidates(family_candidates):
    existing = {item.get("risk_family") for item in family_candidates or [] if isinstance(item, dict)}
    output = list(family_candidates or [])
    required = [
        {
            "risk_family": "rare_composite_risk",
            "score": 0,
            "reason": ["reserved rare slot: model should imagine one uncommon but physically plausible risk"],
        },
        {
            "risk_family": "weather_visibility_risk",
            "score": 0,
            "reason": ["reserved weather perturbation slot: sudden night / visibility loss"],
        },
    ]
    for item in required:
        if item["risk_family"] not in existing:
            output.append(item)
    return output
...
def strip_l1_for_output(risk, family_candidates):
    ...
    allowed_families = {item.get("risk_family") for item in family_candidates or [] if isinstance(item, dict)}
    allowed_families.update(KNOWN_L1_FAMILY_IDS)
    if cleaned.get("risk_family") not in allowed_families:
...
def normalize_risks(parsed, l0_state, scene_risk_candidates=None):
    ...
    for risk in risks:
        if not isinstance(risk, dict):
            risk = {"name": str(risk)}
        if risk.get("risk_family") == "rare_composite_risk" and rare_risk is None:
            rare_risk = risk
            continue
        if risk.get("risk_family") == "weather_visibility_risk":
            continue
        if len(normalized) >= LIKELY_L1_NODE_COUNT:
            continue
        ...
        risk["rank"] = len(normalized) + 1
        cleaned = strip_l1_for_output(risk, scene_risk_candidates or [])
        if cleaned.get("risk_family") in FORCED_L1_FAMILY_IDS:
            continue
        normalized.append(cleaned)

    if rare_risk is None:
        for risk in risks:
            if not isinstance(risk, dict):
                continue
            try:
                is_rank3 = int(risk.get("rank")) == 3
            except (TypeError, ValueError):
                is_rank3 = False
            if is_rank3:
                rare_risk = dict(risk)
                rare_risk["risk_family"] = "rare_composite_risk"
                break

    if len(normalized) < LIKELY_L1_NODE_COUNT:
        fallback_likely = fallback_risks_from_state(l0_state)[:LIKELY_L1_NODE_COUNT]
        for risk in fallback_likely:
            if len(normalized) >= LIKELY_L1_NODE_COUNT:
                break
            normalized.append(strip_l1_for_output(risk, scene_risk_candidates or []))

    tail = forced_l1_tail_risks(l0_state)
    if isinstance(rare_risk, dict):
        rare_risk.setdefault("level", "L1")
        rare_risk.setdefault("risk_type", "物理风险推测")
        rare_risk.setdefault("visibility", "纯推测")
        rare_risk.setdefault("risk_level", "低")
        rare_risk.setdefault("risk_family", "rare_composite_risk")
        rare_risk.setdefault("boundary", "L1只识别风险家族和薄弱环节，不生成触发事件、risk_type、动作原语或具体扰动物体")
        rare_cleaned = strip_l1_for_output(rare_risk, scene_risk_candidates or [])
        rare_cleaned["risk_family"] = "rare_composite_risk"
        tail[0] = rare_cleaned
    normalized = normalized[:LIKELY_L1_NODE_COUNT] + tail
    if len(normalized) != L1_NODE_COUNT:
        raise ValueError(f"L1 output must contain exactly {L1_NODE_COUNT} risks, got {len(normalized)}")

    for idx, risk in enumerate(normalized[:L1_NODE_COUNT], start=1):
        risk["rank"] = idx
    return normalized[:L1_NODE_COUNT]
...
def main():
    ...
    risk_family_candidates = ensure_required_family_candidates(family_candidates_from_scene_candidates(internal_scene_risk_candidates))
