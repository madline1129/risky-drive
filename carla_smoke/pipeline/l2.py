...
- 每个 L1 可能带有 risk_family、risk_type_id、primary_action_primitive_id 或 risk_library_candidate；L2 必须继承它们，并在该风险家族/类型下选择主要触发动作。
  ...
  "trigger_event_hypotheses": [
    {
      ...
      "risk_type_id": "继承或确认后的风险类型ID",
      "primary_trigger_action_id": "主要触发动作原语ID，优先使用L1的primary_action_primitive_id",
...
- 必须继承 L1 的 risk_family/risk_type_id/primary_action_primitive_id；如需调整，只能在 L1 的风险候选内调整，并在 mechanism 说明原因。
...
def inherit_actor_context(event, risk):
    ...
    for source_key, target_key in (
        ...
        ("legacy_scenario_type", "legacy_scenario_type"),
        ("primary_action_primitive_id", "primary_trigger_action_id"),
    ):
        if risk.get(source_key) is not None and target_key not in event:
            event[target_key] = risk[source_key]
    if isinstance(risk.get("risk_library_candidate"), dict) and "risk_library_candidate" not in event:
        event["risk_library_candidate"] = risk["risk_library_candidate"]
    return event
